...
from reid.trainers_clip import Trainer


def get_data(dataname, data_dir, model, matcher, last_epoch, save_path, args):
    ...
    train_loader = DataLoader(
        ...
        sampler=GraphSampler(dataset.train, train_path, test_transformer, model, matcher, args.batch_size, args.num_instance,
                    not args.gs_rerank_off, args.gs_pre_epochs, last_epoch, save_path, args.gs_verbose),
        pin_memory=True)
...
def main(args):
    ...
    dataset, num_classes, train_loader, _, _ = get_data(args.dataset, args.data_dir, model, matcher, start_epoch-1, save_path, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.001,
                        help="Learning rate of the new parameters. For pretrained "
                             "parameters it is 10 times smaller than this. Default: 0.001.")
    ...
    parser.add_argument('--clip_value', type=float, default=512, help="the gradient clip value, default: 512")
    # graph sampler
    parser.add_argument('--num_instance', type=int, default=4, help="the number of instance per class in a batch, default: 4")
    parser.add_argument('--gs_pre_epochs', type=int, default=1, help="the number of starting epochs for the random PK sampler, default: 1")
    parser.add_argument('--gs_rerank_off', action='store_true', default=False, help="turn off the reranking for the graph distance, default: False")
