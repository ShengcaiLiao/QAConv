...
def pairwise_distance(gal_fea, prob_fea, qaconv_layer, gal_batch_size=16, prob_batch_size=4096):
    with torch.no_grad():
        qaconv = torch.nn.DataParallel(QAConv(prob_fea.cuda(), qaconv_layer)).cuda().eval()
        score = qaconv(gal_fea.cuda(), gal_batch_size, prob_batch_size)
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, gallery_loader, testset, qaconv_layer, gal_batch_size=16, prob_batch_size=4096,
        ...
        with torch.no_grad():
            ...
            dist_rerank[:num_prob, :num_prob] = pairwise_distance(prob_fea, prob_fea, qaconv_layer, gal_batch_size, prob_batch_size)
            # dist_rerank[num_prob:, num_prob:] = pairwise_distance(gal_fea, gal_fea, loss_layer, gal_batch_size, prob_batch_size)  # out of memory
            gal_part_size = num_prob
            for i in range(0, num_gal, gal_part_size):
                i_end = min(num_gal, i + gal_part_size)
                dist_rerank[num_prob:, num_prob + i: num_prob + i_end] = \
                    pairwise_distance(gal_fea, gal_fea[i: i_end], qaconv_layer, gal_batch_size, prob_batch_size)
